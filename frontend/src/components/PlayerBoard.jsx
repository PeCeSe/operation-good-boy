...
export function StagingToken({ token }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`w-11 h-11 shrink-0 flex items-center justify-center select-none transition-all ${
...
function DraggableHandCard({ card, position, zIndex, onBringToFront, isMe, isSelected, isPreview = false, onToggleSelect, following = false, dragDelta = null, remoteOffset = null }) {
  ...
  // Appear animation on first render
  ...
  // Drag offset folded into left/top (not transform) so it reconciles cleanly on drop:
  //  - grabbed card  → dnd-kit's `transform`
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1.5 shrink-0">
        ...
        <div ref={setDrawRef} className="relative" style={{ width: 182, height: 264 }}>
          ...
          <button
            ...
            className={`absolute rounded-xl border-2 flex items-center justify-center select-none overflow-hidden transition-all ${
              isOverDraw
                ...
                : drawCount > 0
                ? "border-brown"
                : "border-dashed border-ink-300/50"
            } ${isMe && drawCount > 0 ? "hover:brightness-110 cursor-pointer active:scale-95" : "cursor-default"} ${isDrawDragging ? "opacity-40" : ""}`}
            style={{ width: 176, height: 258, top: drawCount > 2 ? 0 : drawCount > 1 ? 3 : 6, left: drawCount > 2 ? 0 : drawCount > 1 ? 3 : 6, touchAction: "none" }}
      ...
      <div ref={handCanvasRef} className="flex-1 relative" style={{ minHeight: 320 }} onPointerDown={onCanvasPointerDown}>
        ...
        {handLayout === "sorted" ? (
          ...
          hand?.map((card, i) => {
            // A selected sibling of an active multi-drag — follows the grabbed card's delta
            const following = activeDragCardId != null
...
function HandArea({ hand, drawPile, discardPile, peekCard, isMe, serverCardPositions, serverZOrder, handLayout = "tidy", serverCardOrder = [], targetPlayerId, viewerCursors = {}, liveDrag = null, myColor, myName }) {
  ...
  const handleDragEnd = ({ active, over, delta }) => {
    ...
    const groupIds = isMultiDrag ? [...selectedCards] : [cardId];

    if (over?.id === "inner_draw_pile") {
      groupIds.forEach(id => socket.emit("return_card_to_deck", { cardId: id }));
      if (isMultiDrag) setSelectedCards(new Set());
    } else if (over?.id === "inner_discard_pile") {
      setPendingDiscards(prev => new Set([...prev, ...groupIds]));
